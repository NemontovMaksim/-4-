...
const createCategory = async (req, res, next) => {
  ...
  try {
    ...
    next();
        ...
        res.status(400).send(JSON.stringify({ message: "Ошибка создания category" }));
  }
};
...
const deleteCategory = async (req, res, next) => {
  try {
    ...
    res.setHeader("Content-Type", "application/json");
        res.status(400).send(JSON.stringify({ message: "Ошибка удаления category" }));
  }
};
...
module.exports = {findAllCategories,checkIsCategoryExists,checkEmptyName, createCategory,findCategoryById,updateCategory,deleteCategory}